...
@section('main')
    <h1 class="title">Edit Data Mahasiswa PKL</h1>
    ...
    <form action="{{ route('mhs_pkl.update', $mhsPkl->id_pkl) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="id_mhs" name="id_mhs" required>
                ...
                @foreach ($mahasiswa as $mhs)
                    <option value="{{ $mhs->id_mhs }}" {{ $mhs->id_mhs == $mhsPkl->id_mhs ? 'selected' : '' }}>
                        {{ $mhs->nama }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="judul" name="judul" rows="3">{{ $mhsPkl->judul }}</textarea>
        </div>
            ...
            <select class="form-control" id="ruang_sidang" name="ruang_sidang">
                ...
                @foreach ($ruangans as $ruangan)
                    <option value="{{ $ruangan->id_ruangan }}"
                        {{ $ruangan->id_ruangan == $mhsPkl->ruang_sidang ? 'selected' : '' }}>
            ...
            <select class="form-control" id="id_tmpt_pkl" name="id_tmpt_pkl">
                ...
                @foreach ($tempatPkl as $tempat)
                    <option value="{{ $tempat->id_tmpt_pkl }}"
                        {{ $tempat->id_tmpt_pkl == $mhsPkl->id_tmpt_pkl ? 'selected' : '' }}>
                        {{ $tempat->nama }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="tahun_pkl" name="tahun_pkl" value="{{ $mhsPkl->tahun_pkl }}"
                placeholder="YYYY" />
        </div>
            ...
            <select class="form-control" id="dosen_pembimbing" name="dosen_pembimbing">
                ...
                @foreach ($dosens as $dosen)
                    <option value="{{ $dosen->id_dosen }}"
                        {{ $dosen->id_dosen == $mhsPkl->dosen_pembimbing ? 'selected' : '' }}>
                        {{ $dosen->nama }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="dosen_penguji" name="dosen_penguji">
                ...
                @foreach ($dosens as $dosen)
                    <option value="{{ $dosen->id_dosen }}"
                        {{ $dosen->id_dosen == $mhsPkl->dosen_penguji ? 'selected' : '' }}>
                        {{ $dosen->nama }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" class="form-control" id="nilai_pembibing_industri"
                name="nilai_pembibing_industri" value="{{ $mhsPkl->nilai_pembibing_industri }}" />
        </div>
        ...
        <div class="mb-3">
            ...
            @if ($mhsPkl->dokument_nilai_industri)
                <small class="form-text text-muted">Current file: {{ basename($mhsPkl->dokument_nilai_industri) }}</small>
            @endif
        </div>
        ...
        <div class="mb-3">
            ...
            @if ($mhsPkl->dokument_pkl)
                <small class="form-text text-muted">Current file: {{ basename($mhsPkl->dokument_pkl) }}</small>
            @endif
        </div>
        ...
        <div class="mb-3">
            ...
            @if ($mhsPkl->dokument_pkl_revisi)
                <small class="form-text text-muted">Current file: {{ basename($mhsPkl->dokument_pkl_revisi) }}</small>
            @endif
        </div>
        ...
        <div class="mb-3">
            ...
            @if ($mhsPkl->dokument_kuisioner)
                <small class="form-text text-muted">Current file: {{ basename($mhsPkl->dokument_kuisioner) }}</small>
            @endif
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" class="form-control" id="tgl_sidang" name="tgl_sidang"
                value="{{ \Carbon\Carbon::parse($mhsPkl->tgl_sidang)->format('Y-m-d\TH:i') }}" />
        </div>

        <div class="mb-3">
            <label for="rekomendasi" class="form-label">Rekomendasi</label>
            <input type="text" class="form-control" id="rekomendasi" name="rekomendasi"
                value="{{ $mhsPkl->rekomendasi }}" />
        </div>

        <div class="mb-3">
            <label for="informasi_detail" class="form-label">Informasi Detail</label>
            <textarea class="form-control" id="informasi_detail" name="informasi_detail" rows="3">{{ $mhsPkl->informasi_detail }}</textarea>
        </div>
