...
@section('main')
    <h1 class="title">
        <i class="bi bi-pencil-square"></i> Tambah Log Book PKL Mahasiswa
    </h1>

    <form action="{{ route('mhs_pkl_log_book.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="id_pkl" id="id_pkl">
                ...
                @foreach ($mhsPkl as $pkl)
                    <option value="{{ $pkl->id_pkl }}" {{ old('id_pkl') == $pkl->id_pkl ? 'selected' : '' }}>
                        {{ $pkl->nama }} - {{ $pkl->judul }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="tgl_kegiatan_awal" name="tgl_kegiatan_awal"
                value="{{ old('tgl_kegiatan_awal') }}" required>
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="tgl_kegiatan_akhir" name="tgl_kegiatan_akhir"
                value="{{ old('tgl_kegiatan_akhir') }}" required>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="kegiatan" name="kegiatan" rows="4" required>{{ old('kegiatan') }}</textarea>
            @error('kegiatan')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
        ...
        <button type="submit" class="btn btn-primary">
            <i class="bi bi-save"></i> Simpan Log Book
        </button>
