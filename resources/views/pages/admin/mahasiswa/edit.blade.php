                            ...
                            <form method="POST" action="{{ route('mahasiswa.update', $mahasiswa->id) }}" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="jurusan_id" name="jurusan_id" class="select2 form-control @error('jurusan_id') is-invalid @enderror">
                                        ...
                                        <option value="{{ $data->id }}" @if ($mahasiswa->jurusan_id == $data->id)
                                            selected
                                            @endif
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="jekel" name="jekel" required>
                                        <option value="pria" {{ $mahasiswa->jekel == 'pria' ? 'selected' : '' }}>Pria</option>
                                        <option value="wanita" {{ $mahasiswa->jekel == 'wanita' ? 'selected' : '' }}>Wanita</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label>Alamat</label>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="kelas" name="kelas" class="form-control @error('kelas') is-invalid @enderror" placeholder="{{ __('kelas') }}" value="{{ $mahasiswa->kelas }}">
                                </div>
                                <div class="form-group">
                                    <label for="gambar">Foto Profil</label>
                                    <input type="file" name="gambar" class="form-control" accept="image/*" placeholder="{{ __('gambar') }}">{{ $mahasiswa->gambar }}>
                                </div>
