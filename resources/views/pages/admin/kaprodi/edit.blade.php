                        ...
                        <form method="POST" action="{{ route('kaprodi.update', $kaprodi->id) }}" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <select id="jurusan_id" name="jurusan_id" class="select2 form-control @error('jurusan_id') is-invalid @enderror">
                                    ...
                                    <option value="{{ $data->id }}" @if ($kaprodi->jurusan_id == $data->id)
                                        selected
                                        @endif
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="jekel" name="jekel" required>
                                    <option value="pria" {{ $kaprodi->jekel == 'pria' ? 'selected' : '' }}>Pria</option>
                                    <option value="wanita" {{ $kaprodi->jekel == 'wanita' ? 'selected' : '' }}>Wanita</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="gambar">Foto Profil</label>
                                <input type="file" name="gambar" class="form-control" accept="image/*" placeholder="{{ __('gambar') }}">{{ $kaprodi->gambar }}>
                            </div>
