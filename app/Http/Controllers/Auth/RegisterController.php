        ...
        switch ($data['roles']) {
            ...
            case 'dosen':
                ...
                if (!$dosen) {
                    ...
                    $user->delete();
                    return redirect()->back()->with('error', 'NIDN tidak terdaftar sebagai dosen atau kaprodi atau kajur');
                }
                $user->nidn = $data['nidn'];
                ...
                // Update existing Kajur/Dosen record with user_id
                $model = $data['roles'] == 'kajur' ? Kajur::where('nidn', $data['nidn'])->first() : $dosen;
                if ($model) {
                    $model->update(['user_id' => $user->id]);
                }
                break;
