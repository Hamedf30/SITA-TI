...
class ProposalTaController extends Controller
{
    ...
    public function indexForKaprodi()
    {
        ...
        // Jika proposal spesifik diperlukan
        $proposal = $proposal_ta->first(); // Contoh: ambil satu data proposal (sesuaikan logika ini)

        return view('pages.kaprodi.proposalTa.index', compact('proposal_ta', 'dosen', 'proposal'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // // return redirect()->route('proposalTa.index')->with('success', 'File Berhasil ditambahkan');
        // return redirect()->route('pages.kaprodi.proposalTa.index');
    }
    ...
    // }
        ...
        public function pem1(Request $request)
    {
        // Cari Kaprodi berdasarkan user ID
        $kaprodi = Kaprodi::where('user_id', Auth::id())->first();
        
        // Cari Proposal TA berdasarkan mahasiswa yang relevan
        $proposal_ta = ProposalTa::where('id', $request->proposal_ta_id)->first();
        
        if (!$proposal_ta) {
            return redirect()->route('proposalTa.index')->with('error', 'Anda harus meng-upload proposal terlebih dahulu.');
        }

        // Ambil data mahasiswa terkait proposal
        $mahasiswa = $proposal_ta->mahasiswa;

        $this->validate($request, [
            'dosen_id' => 'required|exists:dosens,id',
        ]);

        // Cari atau buat data Pembimbing1
        $pembimbing1 = Pembimbing1::where('mahasiswa_id', $mahasiswa->id)->first();
